...
int main(int argc, char** argv) {
	...
	for (;;) {
		...
		while (true) {
			...
			if ((toupper(_continue) != 'Y') && (toupper(_continue) != 'N')) {
...
//function that checks if the the coordinates are within the image size limit
bool valid_co(cv::Point_<int> tp, cv::Point_<int> bt, int wx, int wy) {
...
//get colour from user input and return scalar value of the colour.
cv::Scalar getColour() {
	cv::Scalar value; 
	char input;
	bool valid = false;
